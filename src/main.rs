...
#[inline]
fn find_subsequence_v3(haystack: &[u8], needle: &Finder) -> Option<usize> {
...
fn main() -> io::Result<()> {
    ...
    let needle = memmem::Finder::new(&[45, 45, 98, 111, 117, 110, 100, 97, 114, 121, 45, 45]);
    ...
    loop {
        let length = {
            ...
            match find_subsequence_v3(&buffer, &needle) {
                Some(index) => {
                    count += 1;
                    ind += index;
